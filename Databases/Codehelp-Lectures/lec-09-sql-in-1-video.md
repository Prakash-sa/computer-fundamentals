...
6. Dierence between SQL and MySQL
...
2. Data can be of dierent types, like INT, CHAR etc.
...
2. e.g., SELECT *  FROM o cers WHERE ocer_name IN ('Lakshay', Maharana Pratap', Deepika);
7. AND/OR/NOT
...
1. What would happen to child entry if parent tables entry is deleted?
...
Data types of two tables can be dierent. Datatypes of corresponding columns from each
...
3. The View and table have one main dierence that the views are definitions built on top of other tables (or views). If any
...
Complex, dicult to understand and implement Comparatively easy to understand and implement.
